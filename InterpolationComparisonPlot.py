...
plot(X, Model_Nearest, linewidth = lw, color = "yellow")
plot(X, Model_Linear, linewidth = lw, color = "red")
plot(X, Model_Sinc, linewidth = lw, color = "blue")
plot(X, Linear_Sinc, linewidth = lw, color = "orange")
plot(X, Nearest_Sinc, linewidth = lw, color = "green")

xlim(0, int(int(X[-1]) * 0.8))
ylim(0.5, 1)

savefig("InterpolationComparisonPlot.png", dpi = 200)
show()
